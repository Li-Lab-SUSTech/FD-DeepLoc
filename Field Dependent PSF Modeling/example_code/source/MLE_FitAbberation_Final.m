function [P,modelOut,err] = MLE_FitAbberation_Final(data,theta0,parameters,shared)
...
map = parameters.map;
data(data<=0) = 1e3*eps;
[newDudt,model] =  kernel_DerivativeAberration_vectorPSF(newTheta,parameters,map);
%need check negative
...
for kk =1:iterations
    ...
    if abs((newErr-oldErr)/newErr)<tolerance
        ...
        [L,U,errFlag] = kernel_cholesky(hessian,NV);
        if errFlag ==0
            oldLambda = newLambda;
            oldUpdate = newUpdate;
