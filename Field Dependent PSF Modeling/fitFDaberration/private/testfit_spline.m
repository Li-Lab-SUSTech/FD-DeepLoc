...
% roifit = (size(teststack,1)+1)/2; %modify by shiwei 2021/12/23
roifit =size(coeff{1},1)+1;%modify by shiwei 2021/12/23
% roifit=51;   %mark tetrapod 17   61
...
%         [P,CRLB, LL,residuals] =CPUmleFit_LM_MultiChannel_R(fitstack,int32(sharedA),iterations,coeffh,single(dT));
        ...
        posbeads.znm(:,k)=znm;  %add by shiwei 20210623
% 
% plot(ax2,P(:,1),P(:,2),'.')
% hold(ax2,'on')
