function [splinefit,indgood,posbeads,shift,testallrois,beadsposxy]=getstackcal_g(beads,p)
...
%     allstackstm=zeros(sstack(1),sstack(2),sstack(3),length(beads))+NaN;
    goodvs=[];
%%test    Unable to perform assignment because the size of the left side is 33-by-33-by-41 and the size of the right side is 33-by-33-by-3.
%     for i=length(beads):-1:1
%         if length(beads(i).stack.framerange)~=41
%             i
%         end
%     end
    for B=length(beads):-1:1
...
%     [corrPSF,shiftedstack,shift,beadgood]=registerPSF3D_g(allstacks,allstackst,struct('sortind',sortinddev,'shiftxy',shiftxy,'framerange',framerange,'alignz',zcorr,'zshiftf0',zshift,'beadfilterf0',false,'status',p.status),{},filenumber(sortinddev));
    [corrPSF,shiftedstack,shift,beadgood,corrPSF_all]=registerPSF3D_g(allstacks,allstackst,struct('sortind',sortinddev,'shiftxy',shiftxy,'framerange',framerange,'alignz',zcorr,'zshiftf0',zshift,'beadfilterf0',false,'status',p.status),{},filenumber(sortinddev));
%     corrPSFr_all = corrPSF_all(1:size(allstacks,1),:,:,:);
    corrPSFr_all = corrPSF_all;
    beadsposxy = cell(1,length(beads));
    for k=length(beads):-1:1
        beadsposxy{k}=beads(k).pos;
    end 
    beadsposxy=beadsposxy(beadgood);
%     for k=length(beads):-1:1
%         tempx(k)=beads(k).pos(1);
%         tempy(k)=beads(k).pos(2);
%     end 
%     beadsposxy=[tempx(beadgood);tempy(beadgood)];
%end add by shiwei 2020/08/14
...
%     lambdax=p.smoothxy/p.cam_pixelsize_um(1)/100000;
    ...
    p.status.String='calculating calibration coefficients in progress';drawnow
...
%     else
        ...
        splinefit.PSF_all={single(corrPSFr_all)};%add by shiwei 2020/08/14
